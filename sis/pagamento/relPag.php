...
$query = "SELECT 
        pagamentos.idPag,
        pagamentos.idUsu, 
        pagamentos.cod_plano,

        usuarios.nome AS nome,
        usuarios.email AS email, 

        plano.titulo_plano AS titulo_plano, 
        plano.valor_plano AS valor_plano 
    FROM pagamentos
    LEFT JOIN usuarios ON pagamentos.idUsu = usuarios.idUsu
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $html .= '
            ...
            <td>' . $row['nome'] . '</td>
            <td>' . $row['email'] . '</td>
            <td>' . $row['titulo_plano'] . '</td>
            <td>R$' . $row['valor_plano'] . ',00</td>';
    }
...
$mpdf->Output(__DIR__ . '/litarUsuario.php', \Mpdf\Output\Destination::INLINE); // GERA E ENVIA O PDF PARA O NAVEGADOR
